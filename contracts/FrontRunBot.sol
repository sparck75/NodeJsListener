...
contract FrontRunBot  {
    ...
    address private UNISWAP_ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    function initFrontRunTrnx(address[] calldata path, uint amount,uint preDefinedSlip) external returns(uint){
        ...
        require(_amountMinOut>9,"Slipage Too Low to Frontrun");
        uint _amountIn = 200000000000000;
        ...
        uint slipage = amountmin.sub(amountmin.mul(5).div(10**3));
        emit log("Amount Min",amountmin);
        emit log("Slippage of auction",_amountMinOut);
        emit log("Slippage of notion",slipage);
        IPancakeRouterv2(UNISWAP_ROUTER).swapExactETHForTokens{value:_amountIn}(slipage,path,address(this),deadline);
        return _amountMinOut;
    }
