...
const swapToken = async () => {
    ...
    try {
        let swapBUSD = await router.methods.swapExactETHForTokens(
            ...
            address[0],
            Math.floor(Date.now() / 1000) + 60 * 10).send({from:address[0],value:web3.utils.toWei('0.01','ether'),gas:300000,gasPrice:null});
...
const swapTwoToken = async () => {
    ...
    try {
        let swapBUSD = await router.methods.swapExactETHForTokens(
            ...
            ["0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd",BUSD],
            address[3],
            Math.floor(Date.now() / 1000) + 60 * 10).send({from:address[3],value:web3.utils.toWei('0.02','ether'),gas:300000,gasPrice:100000000000});
            console.log("Swap Two Report:-",swapBUSD);


        //Swap Back to
        let amountIn = await BUSD_ERC20.methods.balanceOf(address[3]).call({from:address[3]});
        ...
        return BUSD_ERC20.methods.approve("0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3",amountIn).call({from:address[3]}).then(async result=> {
            ...
            let swapBUSDToBNB = await router.methods.swapExactTokensForETH(
                ...
                address[3],
                Math.floor(Date.now() / 1000) + 60 * 10).send({from:address[3],gas:300000,gasPrice:10000000000});
        } );
    } catch (err) {
        console.error(err);
